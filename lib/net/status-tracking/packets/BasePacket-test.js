...
describe('net/status-tracking/packets/BasePacket', function () {
	...
	it('should set subtype from input', function () {
		const expected = 0x03;

		const actual = new BasePacket(0x01, expected);

		assert.equal(actual.subType, expected);
	});
	...
	describe('toBuffer', function () {
		...
		it('should return a buffer 0x22 bytes long', function () {
			const actual = packet.toBuffer().length;

			assert.equal(actual, 0x22);
		});
	});

	describe('fromBuffer', function () {
		const expectedPacketType = packetTypes.PLAYER_STATUS;
		const expectedSubType = 0x03;
...
function runToBufferAssertions(packet) {
	...
	it('should have the packet subtype at 0x20', function () {
		const expected = packet.subType;
...
function runFromBufferAssertions({
	...
	it('should set subtype from packet data', function () {
		const { subType: actual } = decodeFn(buffer);

		assert.equal(actual, expectedSubType);
	});
