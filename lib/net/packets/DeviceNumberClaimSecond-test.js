...
describe('net/packets/DeviceNumberClaimSecond', () => {
	...
	describe('toBuffer', () => {
		...
		it('should have a length of 0x32 bytes', () => {
		...
		it('should have a packet type of 0x02 at 0x0a', () => {
			const packet = new DeviceNumberClaimSecond(deviceTypes.MIXER);
		...
		it('should set a 4 byte ip address starting at byte 0x24', () => {
		...
		it('should set the device type at byte 0x30', () => {
		...
		it(`should set the packet's originating mac address as 6 bytes starting from 0x28`, () => {
		...
		it('should set the wanted device number at byte 0x2e', () => {
			const expected = 2;
		...
		it('should set the packet transmission counter at byte 0x2f', () => {
			const expected = 3;
		...
		it('should set byte 0x31 to 0x01 when using auto assign mode', () => {
			const expected = 0x01;
		...
		it('should set byte 0x31 to 0x02 when trying to claim a specific number', () => {
			const expected = 0x02;
	...
	describe('fromBuffer', () => {
		...
		it('should return an instance of DeviceNumberClaimSecond', () => {
		...
		runFromBufferAssertions({
			buffer: new DeviceNumberClaimSecond(1, expectedDeviceName).toBuffer(),
			decodeFn: DeviceNumberClaimSecond.fromBuffer,
			expectedDeviceName,
			expectedPacketType: packetTypes.NUMBER_CLAIM_SECOND_STAGE
		});

		it('should set deviceType from packet data', () => {
			const expected = deviceTypes.CDJ_XDJ;

			const buffer = new DeviceNumberClaimSecond(expected, 'hehe').toBuffer();
			const { deviceType: actual } = DeviceNumberClaimSecond.fromBuffer(buffer);

			assert.equal(actual, expected);
		});

		it('should set broadcast count from packet data', () => {
			const expected = 3;
			...
			const { broadcastCount: actual } = DeviceNumberClaimSecond.fromBuffer(
				buffer
			);
		...
		it('should set mac address from packet data', () => {
			...
			const { macAddress: actual } = DeviceNumberClaimSecond.fromBuffer(buffer);

			assert.deepEqual(actual, expected);
		});

		it('should set ip address from packet data', () => {
			...
			const { ipAddress: actual } = DeviceNumberClaimSecond.fromBuffer(buffer);

			assert.deepEqual(actual, expected);
		});

		it('should set the wanted device number from packet data', () => {
			const expected = 2;
			...
			const { wantedDeviceNumber: actual } = DeviceNumberClaimSecond.fromBuffer(
				buffer
			);
		...
		it('should set auto assign flag from packet data (test for true)', () => {
			const expected = true;
			...
			const { isAutoAssign: actual } = DeviceNumberClaimSecond.fromBuffer(
				buffer
			);
		...
		it('should set auto assign flag from packet data (test for false)', () => {
