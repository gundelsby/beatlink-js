...
describe('net/packets/DeviceNumberClaimFinal', () => {
	...
	describe('fromBuffer', () => {
		...
		it('should set device number from packet data', () => {
			const expected = 4;

			const buffer = new DeviceNumberClaimFinal('whatevz', expected).toBuffer();
			const { deviceNumber: actual } = DeviceNumberClaimFinal.fromBuffer(
				buffer
			);
		...
		it('should set the packet broadcast count from packet data', () => {
			const expected = 2;

			const buffer = new DeviceNumberClaimFinal('lol', 1, expected).toBuffer();
			const { broadcastCount: actual } = DeviceNumberClaimFinal.fromBuffer(
				buffer
			);
