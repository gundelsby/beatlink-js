 ...
 *
 * @param {import('os').NetworkInterfaceInfoIPv4} interfaceInfo - IP address to bind sockets to
 *
 * @returns {Promise<{NetworkServices, DeviceRegistry, VirtualDevice}>} initaliazed services
 */
export default async function initialize(interfaceInfo) {
	const { address, mac } = interfaceInfo;
	...
	const statusSocketInterfaceMacAddress = mac
		.split(':')
		.map((v) => Number(`0x${v}`));
